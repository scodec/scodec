...
class VariableSizeDelimitedCodecTest extends CodecSuite {
  ...
  "the variableSizeDelimited codecs" should {
    "encode value followed by delimiter" in {
      variableSizeDelimited(constant(0), utf8, 8).encode("test") shouldBe Attempt.successful(
        BitVector('t', 'e', 's', 't', 0)
      )
    }
    ...
    "decode value followed by delimiter" in {
      ...
      variableSizeDelimited(constant(0), ascii, 8).decode(buffer) shouldBe Attempt.successful(
