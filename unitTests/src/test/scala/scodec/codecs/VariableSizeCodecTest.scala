...
class VariableSizeCodecTest extends CodecSuite {
  ...
  "the variableSizeBits/variableSizeBytes codecs" should {
    ...
    "encode size followed by value" in {
      variableSizeBytes(uint8, utf8).encode("test") shouldBe Attempt.successful(
        BitVector(4, 't', 'e', 's', 't')
      )
      variableSizeBits(uint8, utf8).encode("test") shouldBe Attempt.successful(
        BitVector(32, 't', 'e', 's', 't')
      )
    }

    "fail encoding when value is too large to be encoded by size codec" in {
      variableSizeBytes(uint2, utf8).encode("too long") shouldBe Attempt.failure(
        Err.General(
  ...
  "the variableSizeBytes codec" should {
    "fail encoding if the wrapped codec returns a bit vector with length not divisible by 8" in {
      variableSizeBytes(uint8, bool).encode(true) shouldBe Attempt.failure(
        Err.General("failed to encode size of [true]: 1 is not evenly divisible by 8", List("size"))
      )
      variableSizeBytes(uint8, byteAligned(bool)).encode(true) shouldBe Attempt.successful(
