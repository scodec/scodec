...
class VariableSizePrefixedCodecTest extends CodecSuite {
  ...
  "the variableSizePrefixedBits/variableSizePrefixedBytes codecs" should {
    ...
    "encode size followed by value" in {
      variableSizePrefixedBytes(uint8, int32, utf8).encode(2 -> "test") shouldBe Attempt.successful(
        BitVector(4, 0, 0, 0, 2, 't', 'e', 's', 't')
      )
      variableSizePrefixedBits(uint8, int32, utf8).encode(2 -> "test") shouldBe Attempt.successful(
        BitVector(32, 0, 0, 0, 2, 't', 'e', 's', 't')
      )
    }

    "fail encoding when value is too large to be encoded by size codec" in {
      variableSizePrefixedBytes(uint2, int32, utf8).encode(2 -> "too long") shouldBe Attempt
        .failure(
          Err(
