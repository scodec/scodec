...
class LimitedSizeCodecTest extends CodecSuite {
  ...
  "the limitedSizeBits/limitedSizeBytes combinators" should {
    ...
    "fail encoding when value is too large to be encoded by size codec" in {
      ...
      limitedSizeBits(32, utf8).decode(encoded ++ BitVector.low(48)) shouldBe Attempt.successful(DecodeResult("test", BitVector.low(48)))
      limitedSizeBits(24, utf8).encode("test") shouldBe Attempt.failure(Err("[test] requires 32 bits but field is limited to 24 bits"))
    }
  }
}
