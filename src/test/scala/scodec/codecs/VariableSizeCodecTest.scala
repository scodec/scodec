...
class VariableSizeCodecTest extends CodecSuite {
  ...
  "the variableSizeBits/variableSizeBytes codecs" should {
    ...
    "encode size followed by value" in {
      variableSizeBytes(uint8, ascii).encode("test") shouldBe Attempt.successful(BitVector(4, 't', 'e', 's', 't'))
      variableSizeBits(uint8, ascii).encode("test") shouldBe Attempt.successful(BitVector(32, 't', 'e', 's', 't'))
    }

    "fail encoding when value is too large to be encoded by size codec" in {
      variableSizeBytes(uint2, ascii).encode("too long") shouldBe Attempt.failure(Err("[too long] is too long to be encoded: 8 is greater than maximum value 3 for 2-bit unsigned integer"))
    }
