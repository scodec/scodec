...
class VariableSizePrefixedCodecTest extends CodecSuite {
  ...
  "the variableSizePrefixedBits/variableSizePrefixedBytes codecs" should {
    "roundtrip" in {
      roundtripAll(variableSizePrefixedBits(uint8, int32, ascii), Seq(2 -> "", 3 -> "test"))
      roundtripAll(variableSizePrefixedBits(uint8, int32, ascii, 10), Seq(2 -> "", 3 -> "test"))
    }

    "encode size followed by value" in {
      variableSizePrefixedBytes(uint8, int32, ascii).encode(2 -> "test") shouldBe Attempt.successful(BitVector(4, 0, 0, 0, 2, 't', 'e', 's', 't'))
      variableSizePrefixedBits(uint8, int32, ascii).encode(2 -> "test") shouldBe Attempt.successful(BitVector(32, 0, 0, 0, 2, 't', 'e', 's', 't'))
    }

    "fail encoding when value is too large to be encoded by size codec" in {
      variableSizePrefixedBytes(uint2, int32, ascii).encode(2 -> "too long") shouldBe Attempt.failure(Err("[too long] is too long to be encoded: 8 is greater than maximum value 3 for 2-bit unsigned integer"))
    }
