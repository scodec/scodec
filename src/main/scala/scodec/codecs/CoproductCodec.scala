...
final class CoproductCodecBuilder[C <: Coproduct, L <: HList, R] private[scodec] (
   ...
   */
  def as[S](implicit as: CoproductCodecAs[S, R]): CoproductCodecBuilder[C, L, S] =
    xmap(as.from, as.to)
}

/**
 * Typeclass that witnesses that a `CoproductCodecBuilder[C, L, Repr]` can be xmapped in to a `CodecCodecBuilder[C, L, A]`.
 */
@annotation.implicitNotFound("""Could not prove that ${Repr} can be converted to/from ${A}.""")
trait CoproductCodecAs[A, Repr] {
  def to(a: A): Repr
