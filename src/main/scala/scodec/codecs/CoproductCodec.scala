...
final class CoproductCodecBuilder[C <: Coproduct, L <: HList, R] private[scodec] (
  ...
  /**
   * Creates a builder that applies a transformation from `R` to `S` to any codecs generated by the returned builder.
   */
  def as[S](implicit as: CoproductCodecBuilderAs[R, S]): CoproductCodecBuilder[C, L, S] = as(this)
}

/**
 * Typeclass that witnesses that a `CoproductCodecBuilder[C, L, A]` can be coverted to a `CodecCodecBuilder[C, L, B]`.
 */
@annotation.implicitNotFound("""Could not prove that ${A} can be converted to/from ${B}.""")
trait CoproductCodecBuilderAs[A, B] {
  def apply[C <: Coproduct, L <: HList](c: CoproductCodecBuilder[C, L, A]): CoproductCodecBuilder[C, L, B]
}

/** Low priority implicits supporting [[CoproductCodecBuilderAs]]. */
sealed abstract class CoproductCodecBuilderAsLowPriority {

  /** Provides a `CoproductCodecBuilderAs[A, X]` for where `X` is a coproduct whose component types can be aligned with the coproduct representation of `A`. */
  ...
  ): CoproductCodecBuilderAs[X, A] = new CoproductCodecBuilderAs[X, A] {
    def apply[C <: Coproduct, L <: HList](c: CoproductCodecBuilder[C, L, X]): CoproductCodecBuilder[C, L, A] =
      c.xmap(x => gen.from(fromAligned(xToAligned(x))), a => alignedToX(toAligned(gen.to(a))))
  }
}

/** Companion for [[CoproductCodecBuilderAs]]. */
object CoproductCodecBuilderAs extends CoproductCodecBuilderAsLowPriority {
  /** Provides a `CoproductCodecBuilderAs[A, X]` based on a Shapeless `Generic` instance. */
  implicit def fromGeneric[A, Repr, X](implicit gen: Generic.Aux[A, Repr], xToR: X =:= Repr, xToA: Repr =:= X): CoproductCodecBuilderAs[X, A]  = new CoproductCodecBuilderAs[X, A] {
    def apply[C <: Coproduct, L <: HList](c: CoproductCodecBuilder[C, L, X]): CoproductCodecBuilder[C, L, A] =
      c.xmap(x => gen.from(x), a => gen.to(a))
  }
}
