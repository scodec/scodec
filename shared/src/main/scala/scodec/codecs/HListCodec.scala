...
import shapeless._
import ops.hlist.{Init, Last, Length, Prepend, RightFolder, Split}
...
private[scodec] object HListCodec {
  ...
  def append[L <: HList, A, LA <: HList](l: Codec[L], a: Codec[A])(
      implicit
  ...
  def concat[K <: HList, L <: HList, KL <: HList, KLen <: Nat](ck: Codec[K], cl: Codec[L])(
      implicit
  ...
  def flatPrepend[A, L <: HList](codecA: Codec[A], f: A => Codec[L]): Codec[A :: L] =
    new Codec[A :: L] {
  ...
  // TODO: In 1.11+, remove lengthK
  def flatConcat[K <: HList, L <: HList, KL <: HList, KLen <: Nat](
      codecK: Codec[K],
      f: K => Codec[L]
  )(
      implicit
  ...
  def flatAppend[L <: HList, A, LA <: HList, Len <: Nat](codecL: Codec[L], f: L => Codec[A])(
      implicit
      prepend: Prepend.Aux[L, A :: HNil, LA],
  ...
  def apply[L <: HList: *->*[Codec]#λ, M <: HList](
      l: L
  )(implicit folder: RightFolder.Aux[L, Codec[HNil], PrependCodec.type, Codec[M]]): Codec[M] =
...
object ToHListCodec {
  ...
  implicit def mk[I <: HList, L0 <: HList](
      implicit
      allCodecs: *->*[Codec]#λ[I],
