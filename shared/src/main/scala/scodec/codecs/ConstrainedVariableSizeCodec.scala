...
private[codecs] final class ConstrainedVariableSizeCodec[A](sizeCodec: Codec[Long], valueCodec: Codec[A], minSize: Long, maxSize: Long) extends Codec[A] {
  ...
  override def encode(a: A) = valueCodec.complete.encode(a) flatMap { enc =>
    ...
    else Attempt.failure(Err(s"Size out of bounds: $minSizeBits <= $sz <= $maxSizeBits is not true"))
