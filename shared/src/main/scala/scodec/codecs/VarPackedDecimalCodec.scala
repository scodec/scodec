...
private [codecs] final class VarPackedDecimalCodec extends Codec[Long] {
  override def sizeBound = SizeBound.bounded(1L, 9L)

  def encode (i: Long): Attempt[BitVector] = {
    require(i >= 0, "VarPackedBcd cannot encode negative longs")
    runEncoding(i, Attempt.successful(BitVector.empty))
  }
  ...
  private def runDecoding (buffer: BitVector, value: Long): Attempt[DecodeResult[Long]] = {
    if (buffer.size == 0) Attempt.successful(DecodeResult(value, buffer))
