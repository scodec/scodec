...
object DecodingContext {
  ...
  def fromFunction[A](f: BitVector => Attempt[DecodeResult[A]]): DecodingContext[A] =
    new DecodingContext[A] {
  ...
  def set(buffer: BitVector): DecodingContext[Unit] =
    fromFunction(_ => Attempt.successful(DecodeResult((), buffer)))

  /** Context that sets the current buffer to the the result of applying the supplied function to the current buffer. */
  def modify(f: BitVector => BitVector): DecodingContext[Unit] =
    fromFunction(b => Attempt.successful(DecodeResult((), f(b))))

  /** Lifts a value of `Attempt[A]` in to a decoding context. */
  def liftAttempt[A](res: Attempt[A]): DecodingContext[A] =
    fromFunction { bv =>
      res.fold(err => Attempt.failure(err), a => Attempt.successful(DecodeResult(a, bv)))
    }
}
